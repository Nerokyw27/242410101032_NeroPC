...
const initApp = () => {
  // Event listeners — cek null agar aman di halaman yang tidak punya semua elemen
  if (formPc) formPc.addEventListener("submit", handleSubmit);
  if (btnResetForm) btnResetForm.addEventListener("click", cancelEdit);

  // Event delegation pada tabel
  if (tbodyPc) tbodyPc.addEventListener("click", handleTableClick);

  // Pencarian real-time
  if (searchInput) searchInput.addEventListener("input", handleSearch);

  // Filter kategori
  if (filterKategori) filterKategori.addEventListener("change", handleFilter);
  ...
  // Setup validasi real-time (hanya jika form ada)
  if (formPc) setupRealtimeValidation();

  // Setup navigasi
  setupNavigation();

  // Render awal (hanya jika elemen ada)
  if (tbodyPc) renderTable();
  if (statTotalProduk) updateStats();
};
